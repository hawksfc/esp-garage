...
static esp_err_t write_cb(const esp_rmaker_device_t *device, const esp_rmaker_param_t *param, const esp_rmaker_param_val_t val, void *priv_data, esp_rmaker_write_ctx_t *ctx)
{
    ...
    if (strcmp(esp_rmaker_param_get_name(param), ESP_RMAKER_DEF_POWER_NAME) == 0) {
        if (val.val.b) {
            ...
            if (strcmp(device_name, "Open Garage") == 0){
                ESP_LOGI(TAG, "Pulsing OPEN GPIO");
                app_driver_pulse_open();
            } else if (strcmp(device_name, "Close Garage") == 0){
                ESP_LOGI(TAG, "Pulsing CLOSE GPIO");
                app_driver_pulse_close();
            } else if (strcmp(device_name, "Stop Garage") == 0){
                ESP_LOGI(TAG, "Pulsing STOP GPIO");
...
void app_main()
{
    ...
    esp_rmaker_node_t *node = esp_rmaker_node_init(&rainmaker_cfg, "Control Porton", "Sw");
    if (!node) {
     ...
     * parameter.
     */
    esp_rmaker_device_add_param(close_garage_device, esp_rmaker_name_param_create(ESP_RMAKER_DEF_NAME_PARAM, "Close Garage"));
    esp_rmaker_device_add_param(stop_garage_device, esp_rmaker_name_param_create(ESP_RMAKER_DEF_NAME_PARAM, "Stop Garage"));
    esp_rmaker_device_add_param(open_garage_device, esp_rmaker_name_param_create(ESP_RMAKER_DEF_NAME_PARAM, "Open Garage"));

    /* Add the standard power parameter (type: esp.param.power), which adds a boolean param
     * with a toggle switch ui-type.
     */
    esp_rmaker_param_t *close_param = esp_rmaker_power_param_create(ESP_RMAKER_DEF_POWER_NAME, 0);
    esp_rmaker_device_add_param(close_garage_device, close_param);

    esp_rmaker_param_t *stop_param = esp_rmaker_power_param_create(ESP_RMAKER_DEF_POWER_NAME, 0);
    esp_rmaker_device_add_param(stop_garage_device, stop_param);

    esp_rmaker_param_t *open_param = esp_rmaker_power_param_create(ESP_RMAKER_DEF_POWER_NAME, 0);
    esp_rmaker_device_add_param(open_garage_device, open_param);

    /* Assign the power parameter as the primary, so that it can be controlled from the
     * home screen of the phone apps.
     */
    esp_rmaker_device_assign_primary_param(close_garage_device, close_param);
    esp_rmaker_device_assign_primary_param(stop_garage_device, stop_param);
    esp_rmaker_device_assign_primary_param(open_garage_device, open_param);

    /* Add this switch device to the node */
    esp_rmaker_node_add_device(node, close_garage_device);
    esp_rmaker_node_add_device(node, stop_garage_device);
    esp_rmaker_node_add_device(node, open_garage_device);
